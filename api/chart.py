...
def lon_to_sign(lon):
    ...
    idx = int(lon / 30)
    pos = lon % 30.0
    return {'sign':SIGNS[idx],'sym':SIGN_SYM[idx],'element':ELEMENTS[idx],
            'elementName':ELEM_NAMES[idx],'deg':int(pos),'min':int((pos%1)*60),
...
def placidus_house(planet_lon, cusps):
    ...
    for i in range(1, 13):
        ...
        c2 = float(cusps[i % 12 + 1]) % 360.0
        if c1 < c2:
            if c1 <= p < c2: return i
        else:
            if p >= c1 or p < c2: return i
    return 1
...
def calculate_chart(date_str, time_str, lat, lon):
    ...
    local_dt = tz.localize(datetime(year,month,day,hour,minute), is_dst=None)
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            result = calculate_chart(body['date'],body['time'],float(body['lat']),float(body['lon']))
